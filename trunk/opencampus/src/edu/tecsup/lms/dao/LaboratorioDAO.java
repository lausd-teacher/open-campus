...
import edu.tecsup.lms.modelo.TrabajoIndividual;
import edu.tecsup.lms.modelo.Matricula;
import edu.tecsup.lms.modelo.tindividual.TrabajoIndividualInteraccion;
import edu.tecsup.lms.modelo.tindividual.TrabajoIndividualMatricula;
import edu.tecsup.lms.util.Constante;
import edu.tecsup.lms.util.Formato;

public class LaboratorioDAO extends BaseDAO {
	...
	public TrabajoIndividual obtenerTrabajoIndividual(TrabajoIndividual ti)
		...
		try {
			...
			String query = "SELECT T.IDTRABAJO,T.FECHA_ACTIVACION,T.FECHA_ENTREGA,M.USUARIO, "
					...
					+ "FROM CV_LABORATORIO T, CV_MATRICULA M, SEGURIDAD.SEG_USUARIO U, GENERAL.GEN_PERSONA P, CV_UNIDAD D "
	...
	public Collection<TrabajoIndividualMatricula> listarTrabajoPorMatricula(
		...
		try {
			...
			String query = "SELECT T.IDMATRICULA,T.ESTADO,T.NOTA,T.EXPIRADO,M.USUARIO,PKG_CV_UTIL.fx_cv_string_number(M.SECCION) SECCION,P.APEPATERNO,P.APEMATERNO,P.NOMUNO,P.NOMDOS "
					+ "FROM CV_LABORATORIO_NOTA T, CV_MATRICULA M, SEGURIDAD.SEG_USUARIO U, GENERAL.GEN_PERSONA P "
	...
	public void publicarTrabajo(TrabajoIndividual tindividual)
		...
		try {
			// *** CV_LABORATORIO
			String query = "UPDATE CV_LABORATORIO SET FECHA_ACTIVACION=TO_DATE (?, 'DD-MM-YYYY HH24:MI'), FECHA_ENTREGA=TO_DATE (?, 'DD-MM-YYYY HH24:MI'), DESCRIPCION=?, "
			...
			if (1 != stmt.executeUpdate()) {
				log
						.error("Error en publicarTrabajo(TrabajoIndividual tindividual) - CV_LABORATORIO");
				throw new DAOException("No culmino");
			}

			// *** TRABAJO_INDIVIDUAL_NOTA
			query = "INSERT INTO CV_LABORATORIO_NOTA (IDTRABAJO,IDMATRICULA,USUARIO_MOD,FECHA_MOD,ESTADO,EXPIRADO) "
			...
			if (0 == stmt.executeUpdate()) {
				log
						.error("Error en publicarTrabajo(TrabajoIndividual tindividual) - TRABAJO_INDIVIDUAL_NOTA");
				throw new DAOException("No culmino");
			}

			// *** TRABAJO_INDIVIDUAL_INTER
			query = "INSERT INTO CV_LABORATORIO_INTER (IDTRABAJO,IDMATRICULA,INTERACCION,DESCRIPCION,IDMATRICULA_ENVIO,USUARIO_CREACION,FECHA_CREACION,USUARIO_MOD,FECHA_MOD,ARCHIVO_NOMBRE,ARCHIVO_TAMANO) "
			...
			stmt
					.setInt(10,
							Constante.ROL_CAMPUS_AULAVIRTUAL_ESTUDIANTE_MONITOR);
	...
	public void modificarTrabajo(TrabajoIndividual tindividual)
		...
		try {
			...
			if (tindividual.getInteraccion().getArchivoNombre() != null) {
				query = "UPDATE CV_LABORATORIO SET FECHA_ACTIVACION=TO_DATE (?, 'DD-MM-YYYY HH24:MI'), FECHA_ENTREGA=TO_DATE (?, 'DD-MM-YYYY HH24:MI'), DESCRIPCION=?, "
						+ "IDMATRICULA_ENVIO=?, ARCHIVO_NOMBRE=?, ARCHIVO_TAMANO=?, USUARIO_MOD=?, FECHA_MOD=SYSDATE "
						+ "WHERE IDTRABAJO=?";
				stmt = (PreparedStatement) cons.prepareStatement(query);
			...
			if (1 != stmt.executeUpdate()) {
				log
						.error("Error modificarTrabajo(TrabajoIndividual tindividual) - CV_LABORATORIO");
				throw new DAOException("No culmino");
			}

			// *** TRABAJO_INDIVIDUAL_INTER
			if (tindividual.getInteraccion().getArchivoNombre() != null) {
				query = "UPDATE CV_LABORATORIO_INTER SET DESCRIPCION=?, IDMATRICULA_ENVIO=?, USUARIO_MOD=?,FECHA_MOD=SYSDATE, ARCHIVO_NOMBRE=?, ARCHIVO_TAMANO=? "
				...
				query = "UPDATE CV_LABORATORIO_INTER SET DESCRIPCION=?, IDMATRICULA_ENVIO=?, USUARIO_MOD=?,FECHA_MOD=SYSDATE "
						+ "WHERE IDTRABAJO=? AND INTERACCION='1'";
				stmt = (PreparedStatement) cons.prepareStatement(query);
				stmt
						.setString(1, tindividual.getInteraccion()
	...
	public void eliminarTrabajo(TrabajoIndividual tindividual)
		...
		try {
			...
			query = "UPDATE CV_LABORATORIO SET ARCHIVO_NOMBRE=?, ARCHIVO_TAMANO=?, USUARIO_MOD=?, FECHA_MOD=SYSDATE "
					+ "WHERE IDTRABAJO=?";
			stmt = (PreparedStatement) cons.prepareStatement(query);
			...
			// *** TRABAJO_INDIVIDUAL_INTER
			query = "UPDATE CV_LABORATORIO_INTER SET USUARIO_MOD=?,FECHA_MOD=SYSDATE, ARCHIVO_NOMBRE=?, ARCHIVO_TAMANO=? "
					+ "WHERE IDTRABAJO=? AND INTERACCION='1'";
			stmt = (PreparedStatement) cons.prepareStatement(query);
	...
	public void calificarTrabajo(TrabajoIndividualMatricula tMatricula)
		...
		try {
			...
			String query = "UPDATE CV_LABORATORIO_NOTA SET NOTA=?, USUARIO_MOD=?, FECHA_MOD=SYSDATE "
		...
		try {
			...
			if (result.next()) {
				...
				query = "SELECT T.IDMATRICULA_ENVIO,T.INTERACCION,T.DESCRIPCION,T.USUARIO_CREACION,T.FECHA_CREACION,T.USUARIO_MOD,T.FECHA_MOD, "
						...
						+ "FROM CV_LABORATORIO_INTER T, CV_MATRICULA M, SEGURIDAD.SEG_USUARIO U, GENERAL.GEN_PERSONA P "
	...
	public TrabajoIndividualMatricula obtenerMensajeEstadoYExpiro(
		...
		try {
			...
			String query = "SELECT ESTADO,EXPIRADO,NOTA FROM CV_LABORATORIO_NOTA "
					+ "WHERE IDTRABAJO=? AND IDMATRICULA=?";
			cons = (Connection)dataSource.getConnection();
			stmt = (PreparedStatement) cons.prepareStatement(query);
			stmt.setInt(1, matricula.getIdTrabajo());
	...
	public void nuevoMensajeDeDocente(TrabajoIndividualMatricula matricula)
		...
		try {
			...
			String query = "INSERT INTO CV_LABORATORIO_INTER (IDTRABAJO, IDMATRICULA, INTERACCION, "
			...
			if (1 != stmt.executeUpdate()) {
				log
						.error("Error nuevoMensajeDeDocente(TrabajoIndividual tindividual) - CV_LABORATORIO_INTER");
				throw new DAOException("No culmino");
			}

			// *** TRABAJO_INDIVIDUAL_NOTA
			query = "UPDATE CV_LABORATORIO_NOTA SET ESTADO=? WHERE IDTRABAJO=? AND IDMATRICULA=?";
			stmt = (PreparedStatement) cons.prepareStatement(query);
	...
	public void nuevoMensajeDeEstudiante(TrabajoIndividualMatricula matricula)
		...
		try {
			...
			if (!"".equals(interaccion.getArchivoNombre())
				...
				query = "UPDATE CV_LABORATORIO_NOTA SET ESTADO=?, EXPIRADO=? WHERE IDTRABAJO=? AND IDMATRICULA=?";
				stmt = (PreparedStatement) cons.prepareStatement(query);
	...
	public void modificarMensaje(TrabajoIndividualMatricula matricula)
		...
		try {
			...
			if (!"".equals(interaccion.getArchivoNombre())) {
				String query = "UPDATE CV_LABORATORIO_INTER "
			...
			if (1 != stmt.executeUpdate()) {
				log
						.error("Error modificarMensajeDeDocente(TrabajoIndividual tindividual)");
				throw new DAOException("No culmino");
			}
		} catch (SQLException e) {
			log.error(e);
