...
public class AulaVirtualEstudianteMonitorInterceptor extends
		CampusBaseInterceptor {
	...
	protected String intercepcion(ActionInvocation inv, Usuario user)
			throws Exception {
		AulaVirtual aula = (AulaVirtual) ActionContext.getContext().getSession().get(Constante.AULA_ACTUAL);
		if (aula != null && (aula.getRol().getIdrol() == Constante.ROL_CAMPUS_AULAVIRTUAL_RESPONSABLE
				...
				|| aula.getRol().getIdrol() == Constante.ROL_CAMPUS_AULAVIRTUAL_ESTUDIANTE_MONITOR
				|| aula.getRol().getIdrol() == Constante.ROL_CAMPUS_AULAVIRTUAL_ESTUDIANTE)) {
			return inv.invoke();
		}
		log.warn("intercepcion(): " + getInvocationAction(inv) + " - Usuario no es Estudiante Monitor: "+user);
