	...
	var cadena = "asunto="+escape(form.asunto.value)+"&texto="+escape(texto)+"&predecesor="+form.predecesor.value;
		...
		var query = function (oXML) { 
			if(oXML.responseText === 'OK'){
				...
				alert('Hubo un error al intentar crear, se sugiere actualizar la página.\nPosiblemente este usando caracteres no permitidos como las comillas doble.');
			}
		};
		myConn.connect(xGetContextPath()+"/aulavirtual/dialogo/CrearAsync.action", "POST", cadena, query);
	return false;
}
...
function marcarLeido(input,idDialogo){
	...
	var cadena = "idDialogo="+idDialogo+"&owner="+cmd;
	...
	myConn.connect(xGetContextPath()+"/aulavirtual/dialogo/MarcarLeido.action", "POST", cadena, query);
}

function eliminar(idDialogo){
	if(confirm('¿Está seguro que desea eliminar el diálogo?')){
		var cadena = "idDialogo="+idDialogo;
		...
		myConn.connect(xGetContextPath()+"/aulavirtual/dialogo/EliminarAsync.action", "POST", cadena, query);
	}
}
...
function listarSubTertulias(idDialogo,reload){
	...
	if(reload || xGetElementById('fila_cuerpo_'+idDialogo).style.display == 'none'){
		...
		var cadena = "idDialogo="+idDialogo;//+"&destino="+cmd;
		
		var myConn = new XHConn();
		var query = function (oXML) { 
			if(oXML.responseText !== ''){
				var xml = oXML.responseXML;
				if(xml.getElementsByTagName("dialogos").length !== 0){
		...
		myConn.connect(xGetContextPath()+"/aulavirtual/dialogo/CargarTertulia.action", "POST", cadena, query);
...
function showEvents(id,xml){
	...
	//Recorrer el Arreglo
	for (var x = 0; x < xml.getElementsByTagName("dialogo").length; x = x + 1) {
		//Extraer valiables
		var dialogo = xml.getElementsByTagName("dialogo")[x];
		
		var idDialogo = dialogo.getElementsByTagName("idDialogo")[0].childNodes[0].nodeValue;
		...
		td = xCreate('td');
		td.innerHTML =  '<strong>' + asunto + '</strong>';
		tr.appendChild(td);
