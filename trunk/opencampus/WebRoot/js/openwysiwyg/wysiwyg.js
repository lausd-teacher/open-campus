...
//Name              buttonID                 buttonTitle           buttonImage                            buttonImageRollover
  ...
  "paste":          ['Paste',                'Paste',              imagesDir + 'paste.gif',              imagesDir + 'paste_on.gif'],
	...
	"insertinput":    ['InsertInput',          'InsertInput',        imagesDir + 'input_text.gif',        imagesDir + 'input_text_on.gif']
};
...
\* ---------------------------------------------------------------------- */
function generate_wysiwyg(textareaID, wysiwygWidth, wysiwygHeight, hideToolbar2) {
  ...
  var toolbar;
  toolbar =  '<table id="toolbarWY1" cellpadding="0" cellspacing="0" border="0" class="toolbar1" style="width:' + toolbarWidth + 'px;"><tr><td style="width: 6px;"><img src="' +imagesDir+ 'seperator2.gif" alt="" hspace="3"></td>';
    ...
    if (buttonName[i]) {
      ...
      var buttonImage          = buttonObj[2];
		  var buttonImageRollover  = buttonObj[3];
		    ...
		    toolbar += '<td style="width: 22px;"><img src="' +buttonImage+ '" border=0 unselectable="on" title="' +buttonTitle+ '" id="' +buttonID+ '" class="wbutton" onClick="formatText(this.id,\'' + n + '\');" onmouseover="if(className==\'button\'){className=\'buttonOver\'}; this.src=\'' + buttonImageRollover + '\';" onmouseout="if(className==\'buttonOver\'){className=\'button\'}; this.src=\'' + buttonImage + '\';" unselectable="on" width="20" height="20"></td>';
	    }
    }
    i++;
  }
  ...
  var toolbar2;
  toolbar2 = '<table id="toolbarWY2" cellpadding="0" cellspacing="0" border="0" class="toolbar2" style='+((hideToolbar2)?'display:none;':'')+' width:' + toolbarWidth + 'px;"><tr><td style="width: 6px;"><img src="' +imagesDir+ 'seperator2.gif" alt="" hspace="3"></td>';
      ...
      var buttonImage          = buttonObj[2];
		  ...
		  if (buttonName2[j] == "seperator") {
		    ...
		    toolbar2 += '<td style="width: 22px;">';
				...
				toolbar2 += '<span id="textMode' + n + '"><img src="' +imagesDir+ 'view_text.gif" border=0 unselectable="on" title="viewText"          id="ViewText"       class="wbutton" onClick="formatText(this.id,\'' + n + '\');" onmouseover="if(className==\'button\'){className=\'buttonOver\'}; this.src=\'' +imagesDir+ 'view_text_on.gif\';"    onmouseout="if(className==\'buttonOver\'){className=\'button\'}; this.src=\'' +imagesDir+ 'view_text.gif\';" unselectable="on"  width="20" height="20"></span>';
	      toolbar2 += '</td>';
			}
	    else {
  ...
  toolbar2 += '<td>&nbsp;</td></tr></table>';  
	...
	var iframe = '<table id="contenidoWY" cellpadding="0" cellspacing="0" border="0" style="width:' + wysiwygWidth + 'px; height:' + wysiwygHeight + 'px;border: 1px solid #000000;"><tr><td valign="top">\n'
...
function formatText(id, n, selected) {
  ...
  document.getElementById("wysiwyg" + n).contentWindow.focus();
	...
	var formatIDs = new Array("FontSize","FontName","Bold","Italic","Underline","Subscript","Superscript","Strikethrough","Justifyleft","Justifyright","Justifycenter","InsertUnorderedList","InsertOrderedList","Indent","Outdent","ForeColor","BackColor","InsertImage","InsertTable","CreateLink","InsertInput");
...
function insertInputText(n){
	//if(limitInputText<4){
	
	  	var browserName = navigator.appName;
  		if (browserName == "Microsoft Internet Explorer") {	  
  			insertHTML("<input type='text' style='width:100px;'/>", n);
  		}else{
  			var value = '';
  			while(value == ''){
  				value = prompt("Ingrese la respuesta para esta caja.", "");
  				if(value == '')
  					alert('Debe ingresar un valor para la caja de texto.');
  			}
  			
  			if(value != null){
  				insertHTML("<input type='text' style='width:100px;' value='"+value+"'/>", n);
  			}
  		}
	  		
  		//limitInputText++;
  	/*}else{
  		alert("Llegó al límite de cajas de texto (4).");
  	}*/
}
...
function updateTextArea(n) {
  document.getElementById(n).value = document.getElementById("wysiwyg" + n).contentWindow.document.body.innerHTML.replace(new RegExp('<script[^>]*>([\\S\\s]*?)<\/script>', 'img'), '');
};