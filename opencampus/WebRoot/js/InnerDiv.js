...
var INNERDIV = {
	maxZ : 0,
	profundidad : [],
	// Parametros (idVentana,posX,posY,ancho,alto,tituloVentana,EventoLuegoDeCargar,sinEfecto,estiloPropio)
	newInnerDiv : function (id,x,y,w,h,miUrl,titulo,fnDone,effectNone,miCSS,fnClose){
		...
		if (!INNERDIV.existe(id)){
			...
			//*********************************************** contenedor principal ********************************
				...
				if(!miCSS){
					...
					nuevoDiv.style.border = "3px outset";
				}
			...
			//******************************************** barra de la ventana ************************************
				...
				barra.style.backgroundColor = "#4B78D3";
				barra.style.margin = "0px";
				barra.style.padding = "3px";
				barra.style.height = "13px";
				barra.className = "innerDiv_Title";
			...
			//********************************************* boton de cerrar ***************************************
				...
				//boton.setAttribute("src","../img/icon_salir_x.gif");
				//boton.setAttribute("border","0");
				boton.innerHTML = "Cerrar X";
				boton.id = "btn_cerrar";
			...
			//************************************ scrollcontainer *************************************
				...
				scroll_container.style.backgroundColor = "#FFF";
			...
			if(miUrl != null){
				if(miUrl.indexOf('div:') == 0){
					var divBlock = document.getElementById(miUrl.substring(4));
					if(divBlock){
						scroll_content.appendChild(divBlock);
					}
				}else if(miUrl.indexOf('inner:') == 0){
					var divBlock = document.getElementById(miUrl.substring(6));
					if(divBlock){
						scroll_content.innerHTML = divBlock.innerHTML;
					}
				}else{
					CargarContenido(scroll_content.id,miUrl,null,fnDone,effectNone);
				}
			}
		}
	},
   ...
   cerrar : function(mi_id){
	...
	cargar : function (id,miUrl,params,fnDone,effectNone){
			...
			if(INNERDIV.profundidad.length>0){
				...
				if (document.getElementById(id)!=null){
					...
					/* No necesario, mi elección fue hacerlo manualmente
					...
					CargarContenido(mi_ventana.id,miUrl,params,fnDone,effectNone);
				}
	}
	this.className = id;
...
function CargarContenido(targetId,url,params,fnDone,effectNone) {
	target= document.getElementById(targetId);
	if(!effectNone)
		target.innerHTML = '<p class="load">Cargando/Loading...</p>';
	// Esta librería hace uso de otra llamada: XHConn.js (A Simple XMLHTTP Interface Library)
	...
	var query = function (oXML) { 
		...
		if(!effectNone){
			var fader = new Fadomatic(target.parentNode.parentNode, 10, 0, 0,100);
				fader.fadeIn();
		}
		if(fnDone) fnDone();
	};
