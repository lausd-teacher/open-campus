...
Object.extend(CampusVirtual.Portal.prototype, {lastEvent:null, widgets:null, columns:null, initialize:function (columns, options) {
	this.options = Object.extend({url:obtenerContextPath()+"/admin/portal/GrabarPortalGestionar.action", onOverWidget:null, onOutWidget:null, onChange:null, onUpdate:null, removeEffect:Element.remove, accept:null}, options);
...
var s_s_check =null;
function modificarCheck(s_check,s_accion,s_servicio){	
	if(s_s_check==null){
		s_s_check = s_check;
		var valors = (s_check.checked==true)?'1':'0';
		new Ajax.Request("/opencampus/admin/portal/GrabarPortalAccionGestionar.action",{parameters:"accion="+s_accion+"&estado="+valors+"&servicio="+s_servicio,onSuccess: resultadoanadirServicio});
	}else{
		window.alert('Ummm estos momentos se esta realizando una petici&oacute;n.');
	}
}

function resultadoanadirServicio(s_transport){
	var s_check = s_s_check;
	s_s_check =null;
