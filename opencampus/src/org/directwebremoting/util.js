 ...
 */
 ...
 /*revisar, no utilizado*/
dwr.util.br = function(parent,where){
	var br = document.createElement('br');
	br.insertBefore(parent,where);
};

dwr.util.removeNode = function(ele){	
	dwr.util._temp = dwr.util.byId(ele);
	if (dwr.util._temp) { 
...
dwr.util.clonNode = function(ele,referenceNode,paren, options) {
  ele = dwr.util._getElementById(ele, "clonNode()");
  if (ele == null) return null;
  if (options == null) options = {};
  var clone = ele.cloneNode(true);
  if (options.idPrefix || options.idSuffix) {
  ...
  document.getElementById(paren).insertBefore(clone, referenceNode.nextSibling);
  
  return clone;
};

dwr.util.desaparecemelo = function(){
	document.getElementById('avisito').style.visibility='hidden';
};

/*el eskeip*/
dwr.util.onEscape = function(event, action){
  if (!event) event = window.event;
  if (event && event.keyCode && event.keyCode == 27) action();
}
